...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['productName'])) {
    ...
    foreach ($combinations as $combination) {
        $stockQuantity += intval($combination['quantity']);
    }

    if (empty($productName) || $price <= 0 || $categoryId <= 0 || $stockQuantity <= 0) {
        echo '<script>
            Swal.fire({
                icon: "error",
                title: "Lỗi!",
                text: "Tên sản phẩm, giá, danh mục và số lượng không được để trống hoặc không hợp lệ!",
    ...
    try {
        ...
        foreach ($combinations as $combination) {
            ...
            $quantity = intval($combination['quantity']);
            ...
            <tbody>
                ...
                $products = $db->select("
                    SELECT p.*, c.name AS category_name,
                           GROUP_CONCAT(CONCAT(pc.color, ' - ', pc.size, ' (', pc.quantity, ')') SEPARATOR '<br>') AS combinations
                    FROM products p
                ...
                ?>
                    <tr>
                        ...
                        <td>
                            <button class="btn btn-warning editProductButton" data-product-id="<?= $product['product_id'] ?>">Sửa</button>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateTotalStock() {
        ...
        hiddenCombinations.value = JSON.stringify(combinations.map(combination => {
            const quantityInput = document.getElementById(`quantity_${combination}`);
            const priceInput = document.getElementById(`price_${combination}`);
            return {
    ...
    document.getElementById('addCombinationButton').addEventListener('click', function() {
        ...
        if (!combinations.includes(combination)) {
            ...
            div.innerHTML = `
                ...
                <input type="number" class="form-control me-2" id="price_${combination}" placeholder="Giá" min="0" value="0" style="width: 100px;">
            ...
            // Gắn sự kiện khi số lượng hoặc giá thay đổi
            document.getElementById(`quantity_${combination}`).addEventListener('input', updateTotalStock);
            document.getElementById(`price_${combination}`).addEventListener('input', updateTotalStock);
